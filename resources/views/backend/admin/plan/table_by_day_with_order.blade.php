...
@if(isset($details_days))
    @foreach($details_days as $j => $detail)
        <div>
            @switch(true)
                @case($j == 0 and $detail->planDetail->recipe->recipe_type_id == \App\Models\RecipeType::getIdByName('Desayuno/Merienda'))
                    <h3>DESAYUNO</h3>
                @break

                @case(($j == 2 || $j == 3 || $j == 4) and $detail->planDetail->recipe->recipe_type_id == \App\Models\RecipeType::getIdByName('Desayuno/Merienda'))
                    <h3>MERIENDA</h3>
                @break

                @case(($j == 1 || $j == 3) and $detail->planDetail->recipe->recipe_type_id == \App\Models\RecipeType::getIdByName('Colacion'))
                    <h3>COLACIÓN</h3>
                @break

                @case(($j == 1 || $j == 2) and $detail->planDetail->recipe->recipe_type_id == \App\Models\RecipeType::getIdByName('Almuerzo/Cena'))
                    <h3>ALMUERZO</h3>
                @break

                @case(($j == 3 || $j == 4 || $j == 5) and $detail->planDetail->recipe->recipe_type_id == \App\Models\RecipeType::getIdByName('Almuerzo/Cena'))
                    <h3>CENA</h3>
                @break
            @endswitch
        </div>
