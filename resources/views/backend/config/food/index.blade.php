...
@section('title', app_name() . ' | Administración de Alimentos')
        ...
        <div class="row">
            <div class="col-sm-5">
                <h5 class="card-title mb-0">
                    Administración de Alimentos <small class="text-muted">Alimentos Activos</small>
                </h5>
            </div><!--col-->

            <div class="col-sm-7 pull-right">
                @include('backend.config.food.includes.header-buttons')
            </div><!--col-->
        </div><!--row-->
...
@endsection
@section('modal-yield')
    <div class="modal fade" id="modalCopyFood" role="dialog" aria-labelledby="myModalLabel" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Copiar Alimento</h4>
                ...
                <div class="modal-footer">
                    ...
                    <button id="btnGuardar" class="btn btn-primary" type="button" onclick="copyFood(event)">Guardar</button>
                </div>
            </div>
        </div>
    </div>
...
@push('after-scripts')
    ...
    <script>
        ...
        function modalCopyFood(event,element){
            event.preventDefault();
            var name =  element.data('name');
            var url  =  element.data('url');
            $("#old_name").val(name);
            $("#url_food").val(url);
            $("#modalCopyFood").modal('show');
        }

        function copyFood(event){
            event.preventDefault();
